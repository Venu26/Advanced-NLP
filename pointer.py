...
class Decoder(nn.Module):
    ...
    def forward(self,x,encoder_states,hidden,cell,dicting):
        ...
        for i in range(len(encoder_states[0])):
            f = torch.cat((hidden[0][0],encoder_states[0][i]))
        ...
        return predictions,hidden,cell,Pw
...
class Seq2Seq(nn.Module):
    ...
    def forward(self,source,target,features):
        ...
        for t in range(1,target_len):
            ...
            output,hidden,cell,Pw =self.decoder(p,encoder_states,hidden,cell,dicting)
